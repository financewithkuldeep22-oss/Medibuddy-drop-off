...
// =========================================================
// 2. MENU & APP SETUP (UPDATED WITH DROPZONE)
// =========================================================
function onOpen() {
  SpreadsheetApp.getUi().createMenu('🚀 Redcliffe Manager')
      .addItem('📊 Open Live Dashboard', 'showDashboard')
      .addSeparator()
      .addToUi();
}
...
function showDashboard() {
  ...
  template.userEmail = Session.getActiveUser().getEmail(); // 🚀 Jadoo: User ki original ID pakad li
  
  const html = template.evaluate()
      .setTitle('Redcliffe x MediBuddy Drop-off') 
      .setWidth(1380).setHeight(800).setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL);
  SpreadsheetApp.getUi().showModalDialog(html, 'Redcliffe x MediBuddy Drop-off'); 
}

function doGet() {
  const template = HtmlService.createTemplateFromFile('Index');
  template.isSheetContext = false; // Web App se khola hai
  template.userEmail = "";
  
  return template.evaluate()
      .setTitle('Redcliffe AI Tracker').setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL)
...
function updateRecord(city, oldBookingId, patientName, newStatus, newRemarks, newBookingId, reqIdsStr, newAge, newGender, newFbs, newColTime, userName) {
  ...
  // 🚀 THE FIX: Google ko data index karne ke liye 1.5 seconds ka buffer time do
...
function getAuditAnalytics(startStr, endStr) {
  ...
  allSheets.forEach(sheet => {
    if (sheetsToIgnore.includes(sheet.getName())) return;
...
// =========================================================
// 🚀 SHEET-TO-DASHBOARD REALTIME PING (FIREBASE FIXED)
// =========================================================
function notifyDashboards() {
  try {
    // 1. Apni Firebase Database Secret yahan daalein
    var FIREBASE_SECRET = "e7tyFPQijOcB0uikRUei5kWbdxjLElWvabW1wVuU"; 
    
    // 2. Exact Path: /global_sync/last_update.json par auth ke saath hit karein
    var dbUrl = "https://redcliffelabsxmedibuddy-default-rtdb.firebaseio.com/global_sync/last_update.json?auth=" + FIREBASE_SECRET;
    
    // 3. Sirf timestamp bhejna hai (Frontend yahi expect kar raha hai)
    var payload = JSON.stringify(new Date().getTime());
    
    var options = {
      "method": "put", // 'patch' ki jagah 'put' taaki value direct replace ho
      "contentType": "application/json",
      "payload": payload,
      "muteHttpExceptions": true
    };
    
    // Sheet direct Firebase ko ping bhejegi
    var response = UrlFetchApp.fetch(dbUrl, options);
    // Logger.log(response.getContentText()); // Agar debug karna ho toh isey uncomment karein
  } catch(e) {
...
function manualAttachTRF(city, rid, base64Data, mimeType) {
    try {
        ...
        if (!sheet) return "❌ Sheet not found: " + city;
        ...
        return "❌ DriveApp Error: " + e.message;
    }
}
...
function manualAttachPendingTRF(rowId, bookingId, link) {
    const ss = SpreadsheetApp.openById(OLD_SHEET_ID);
    const ignoreList = ["Dashboard", "Master Data", "Dropdowns", "Instructions", "Pending List", "Comments_DB"];
    let isAttached = false;
    for (let sheet of ss.getSheets()) {
        if (ignoreList.includes(sheet.getName())) continue;
        const lastRow = sheet.getLastRow();
        if (lastRow < 2) continue;
        const map = getColumnMap(sheet);
        if (!map || map.bookingId === -1) continue;
        const lastCol = sheet.getLastColumn();
        const data = sheet.getRange(1, 1, lastRow, lastCol).getValues();
        let cTrf = map.trf > -1 ? map.trf : -1;

        for (let i = 1; i < data.length; i++) {
            if (String(data[i][map.bookingId]).trim().toLowerCase() === bookingId.toLowerCase()) {
                if (cTrf === -1) { cTrf = lastCol; sheet.getRange(1, cTrf + 1).setValue("TRF Link"); }
...
function getTrendAnalyticsData(startStr, endStr) {
  try {
    ...
    // 3. API Tracking Database Cache (For Speed)
    ...
    if (commentSheet) {
        ...
        if(cLastRow >= 2) {
            ...
            for(let i=0; i<cData.length; i++) {
                let logBId = String(cData[i][1]).trim();
                if(logBId && String(cData[i][3]).includes("[API-MATCHED]")) {
                    apiMatchedIds.add(logBId);
                }
            }
        }
    }
    ...
    allSheets.forEach(sheet => {
        ...
        data.forEach(row => {
            ...
            // 🚀 THE FIX: ROBUST DATE PARSING FOR CHARTS
            ...
            if (Object.prototype.toString.call(dateVal) === '[object Date]') {
                ...
                if (parts.length === 3) {
                    ...
                    let tempD = new Date(dStr);
                    if (!isNaN(tempD.getTime())) rDate = tempD;
                }
            }
            ...
            if (rDate >= start && rDate <= end) {
                let key = `${rDate.getFullYear()}-${("0"+(rDate.getMonth()+1)).slice(-2)}-${("0"+rDate.getDate()).slice(-2)}`;
                if(!stats[key]) return; // If out of bounds
...
                let pName = map.name > -1 && row[map.name] ? String(row[map.name]).trim().toLowerCase() : "";

                if (!bId && !reqId && !pName) return; 

                let cleanId = bId.toLowerCase().replace(/\s+/g, '');
                let status = map.status > -1 && row[map.status] ? String(row[map.status]).trim().toLowerCase() : "";
                
                // --- ALIGN WITH DASHBOARD EXCLUSION RULES ---
                if (cleanId.includes("notcollect") || cleanId.includes("cancel") || cleanId.includes("reject") || cleanId.includes("bloodnot")) return;
                let isToCreate = (cleanId === "" || cleanId === "-" || cleanId === "--" || cleanId === "noid" || cleanId === "na" || cleanId === "n/a");
                if (isToCreate || status === "n/a" || status === "na") return;

                let uniqueId = bId || reqId || pName;
                let dedupKey = `${key}_${city}_${uniqueId}`;

                if (seenPatients.has(dedupKey)) return;
                seenPatients.add(dedupKey);

                // Check 1: PPMC vs RETAIL
                let type = map.type > -1 && row[map.type] ? String(row[map.type]).trim().toUpperCase() : "RETAIL";
                let cat = type.includes("PPMC") ? "PPMC" : "RETAIL";

                // Check 2: API vs MANUAL
                let isApi = false;
                let hasApiText = row.some(cell => String(cell).toUpperCase().replace(/\s+/g, '') === "APIUSER" || String(cell).toUpperCase().replace(/\s+/g, '') === "API_USER");
                if (hasApiText || apiMatchedIds.has(bId) || apiMatchedIds.has(reqId)) isApi = true;
                let sourceCat = isApi ? "API" : "MANUAL";

                if (!stats[key][city]) stats[key][city] = { PPMC: 0, RETAIL: 0, API: 0, MANUAL: 0 };
                
                stats[key][city][cat]++;
                stats[key][city][sourceCat]++;
            }
        });
    });
...
function directAttachTrf(base64Data, filename, city, targetId, aiData, userName) {
  try {
      ...
      const ss = SpreadsheetApp.openById(OLD_SHEET_ID);
      const sheet = ss.getSheetByName(city);
      if (!sheet) return { status: "Error", message: "Sheet not found: " + city };
      const map = getColumnMap(sheet);
      if (!map) return { status: "Error", message: "Column map unavailable for: " + city };
